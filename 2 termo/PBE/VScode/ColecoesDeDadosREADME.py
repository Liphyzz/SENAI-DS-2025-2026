...
print(lista[3]) # Apenas o item contido no índice 3 (não dá pra trabalhar com índices que não se tenha nada armazenado)
print(len(lista)) # Ver o tamanho da lista (do 0 ao 3 -> 4)
lista_ao_contrario = sorted(lista) # Não funciona quando a lista tem bool ou str
print(lista_ao_contrario)
...
x,y,z = tupla # x = "Senai", y = True, z = 33
print(type(tupla))
print(tupla)
print(tupla[3])
print(len(tupla))
...
print(tupla.index("Senai", [1, 3])) # Mesma coisa do index(item) normal, mas nesse caso, começa a analizar no índice 1 e termina no 3
#-------------------------------------------------------------------------------------------------------------------------------#
...
conjunto_sem_duplicatas = set(lista_com_duplicatas) # Converte a lista para conjunto, removendo duplicatas
lista_sem_duplicatas = list(conjunto_sem_duplicatas) # Converte de volta para lista, agora sem duplicatas
print(lista_sem_duplicatas)

# print(conjunto[1]) -> Não funciona (não tem índice)
conjunto.add(False) # Adiciona um item ao conjunto (desde que não)
conjunto.remove("Senai") # Remove um item do conjunto pelo nome do mesmo (Dá erro de KeyError, caso o elemento não exista no conjunto)
conjunto.discard(10) # Mesma função do remove, mas caso o item não esteja no conjunto, ele só não faz nada
...
conjunto.intersection_update(conjunto2) # Atualiza o conjunto, mantendo apenas os itens que são comuns aos dois conjuntos
#...
#-------------------------------------------------------------------------------------------------------------------------------#